 ...
 *
 * PURPOSE:
 *   Forcefully end the named conference via the REST API.
 *   Used by Studio as a cleanup step (e.g. timeout / no agent answered).
 *
 * REQUIRED event params (POST body):
 *   conferenceName  – e.g. "TU_CA…"
 *
 * RESPONSE shape:
 *   { ok: true,  ended: bool, conferenceSid: string|null, reason?: string }
...
exports.handler = async function (context, event, callback) {
  ...
  try {
    ...
    // ── Find the in-progress conference ──────────────────────────────
    ...
    if (confs.length === 0) {
      log('info', 'NO_CONFERENCE_TO_END', { reason: 'no in-progress conference with this name' });
      return callback(null, {
        ok: true,
        ended: false,
        conferenceSid: null,
    ...
    log('info', 'CONFERENCE_ENDED', { conferenceSid: conf.sid });
    log('info', 'END', { outcome: 'ended' });
